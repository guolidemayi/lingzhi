...
//    先清除
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (i != 2 && self.scale) {
            contain.containerView.layer.transform = CATransform3DMakeScale(scaleValue, scaleValue-0.08, 1);
        }
...
- (void)animationWith3D:(CGFloat)scale{
    
    GLD_CarouselCellContain *midImgV = self.containerViews[2];
    GLD_CarouselCellContain *leftImgV = self.containerViews[1];
    GLD_CarouselCellContain *rightImgV = self.containerViews[3];
    
//    [UIView animateWithDuration:2 animations:^{
         midImgV.containerView.layer.transform = CATransform3DMakeScale(MAX(1-scale, scaleValue), MAX(1-scale, scaleValue-0.08), 1);
//    }];
        leftImgV.containerView.layer.transform = CATransform3DMakeScale(scaleValue, scaleValue-0.08, 1);
        rightImgV.containerView.layer.transform = CATransform3DMakeScale(scaleValue, scaleValue-0.08, 1);
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if ([scrollView isEqual:self.scrollView]) {
        ...
        //向右滑动
        if(scrollView.contentOffset.x <= contentW+2){
            //停止计时器
            [self stopScroll];
            //取出最后一个容器
            ...
            //重新开始
            [self startScroll];
//            [self animationWith3D:0.05];
        }else
        
        //向左滑动
        if (scrollView.contentOffset.x >= contentW * 3-2) {
            [self stopScroll];
            //取出第一个元素
            ...
            self.scrollView.contentOffset = CGPointMake(contentW * 2, 0);
            [self startScroll];
//            [self animationWith3D:0.2];
            // 重新至中
        }else{
            if (self.scale) {                
                CGFloat scale = ABS((2*contentW - scrollView.contentOffset.x)/contentW);
                [self animationWith3D:scale];
            }
        }
